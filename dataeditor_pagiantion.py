...
edited_page_data = st.data_editor(edited_data.iloc[start_idx-1:end_idx])

# Update the edited data in the session state
edited_data.iloc[start_idx-1:end_idx] = edited_page_data

# Create navigation buttons
prev_page, next_page = st.columns(2)

if prev_page.button("Previous", disabled=current_page == 1):
    current_page -= 1
